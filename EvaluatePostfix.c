...
int Operation(int a,char chr,int b)
{
    int ret_val = 0;
    switch(chr)
    {
        ...
        case '/':
        {
            ...
            {
                fprintf(stderr,"Denominator can't be 0 .....!!!\n");
                exit(1);
            }
            break;
        }
        ...
        default :
        {
                fprintf(stderr,"Unsupported Operator used ...!!!\n");
                exit(1);
        }
    }
...
int Evaluate(String *postfix,struct StackLinkList **stack,struct StackLinkList **top)
{
    int Index_postfix = 0;
    int operandR      = 0;
    int operandL      = 0;

    while (postfix->chr[Index_postfix]!= '\0')
    {
        if(isOperend(postfix->chr[Index_postfix]))
        {
            push(stack,top,myAtoi(postfix->chr[Index_postfix++]));
        }
        else
        {
            operandR =  pop(stack,top);
            operandL =  pop(stack,top);
            push(stack,top,Operation(operandL,postfix->chr[Index_postfix++],operandR));
        }
    }
...
void ConvertAndEvaluate(String *Infix ,String *postfix)
{
    ...
    postfix->chr[++Index_postfix] = '\0';

    fprintf(stdout,"Postfix   String : %s\n",DisplayString(postfix));
    fprintf(stdout,"Evaluating....!!!!\n");
    fprintf(stdout,"Reselt after Evaluation = %d \n",Evaluate(postfix,&stack,&top));
...
int main()
{
    ...
    ConvertAndEvaluate(Infix ,postfix);
